...
type RelatedEntity = { id: string; label: string; type: TaskRelatedTo };
        ...
        <div>
          ...
          <label className="relative flex rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-4 cursor-pointer hover:border-slate-300 hover:bg-slate-50/80 transition-colors">
            <input
              ...
              multiple
              className="sr-only"
              onChange={(e) => onChange({ ...values, attachmentNames: Array.from(e.target.files ?? []).map((f) => f.name).join(', ') })}
            />
            <div className="flex items-center gap-2 w-full">
              <Paperclip size={18} className="text-slate-400 shrink-0" />
              <span className="text-sm text-slate-500">{values.attachmentNames || 'Click or drag files'}</span>
            </div>
          </label>
        </div>
